...
const TooltipLabel = React.memo(function TooltipLabel({
  ...
  label?: React.ReactNode
  labelFormatter?: (value: React.ReactNode, payload: readonly TooltipPayloadEntry[]) => React.ReactNode
  ...
  const [item] = payload
  const pi = item as { dataKey?: string; name?: string }
  const key = `${labelKey ?? pi?.dataKey ?? pi?.name ?? "value"}`
