...
export default function LogPage() {
  ...
  const [foods, setFoods] = useState<FoodEntry[]>([
    { id: "1", foodName: "", servingSize: 0, calories: 0, carbs: 0, protein: 0, fats: 0 },
  ]);
  ...
  useEffect(() => {
    import("../../actions").then(({ getRecentFoods, getFavoriteFoods }) => {
      Promise.all([getRecentFoods(15), getFavoriteFoods()]).then(([r, f]) => {
        setRecent(r || []);
        setFavorites(f || []);
      }).catch(console.error);
    });
  ...
  const parsePaste = (text: string) => {
    ...
    return { calories: find("calories"), carbs: find("carbs"), protein: find("protein"), fats: find("fats") };
  };

  const addFromSuggestion = (food: SuggestionFood) => {
    const id = Date.now().toString();
    const p100 = food.per100g || (food.servingSize > 0 ? {
  ...
  const handleFoodChange = (foodId: string, e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    const numFields = ["servingSize", "calories", "carbs", "protein", "fats"];
    setFoods(prev => prev.map(f =>
      f.id === foodId ? { ...f, [name]: numFields.includes(name) ? Number(value) : value } : f
    ));
  };

  const toggleExpand = (id: string) => {
    setExpandedFoods(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  };

  const addFood = () => {
    const id = Date.now().toString();
    setFoods(prev => [...prev, { id, foodName: "", servingSize: 0, calories: 0, carbs: 0, protein: 0, fats: 0 }]);
    setExpandedFoods(prev => [...prev, id]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      for (const food of valid) {
        await saveFoodLog({ ...food, date: formData.date, meal: formData.meal });
      }
      router.push("/");
    } catch {
    ...
    <div className="min-h-screen bg-background py-8 px-4">
      <Card className="max-w-xl mx-auto shadow-lg">
        ...
        <CardHeader className="text-center border-b mb-4">
          ...
          <p className="text-muted-foreground font-medium">{formatDate(formData.date)}</p>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <NativeSelect name="meal" value={formData.meal} onChange={handleChange}>
            ...
            <div className="flex justify-between text-sm text-muted-foreground pt-2 border-t">
              <span>Total:</span>
              <span>
                {totals.cal.toFixed(0)} kcal | C:{totals.carb.toFixed(0)} P:{totals.pro.toFixed(0)} F:{totals.fat.toFixed(0)}
              </span>
            </div>
